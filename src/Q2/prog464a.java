    ...
    public static void main(String[] args) {
        try {
            ...
            for (int r = 0; r < mat.length; r++) {
                for (int c = 0; c < mat[0].length-1; c++) { // or can do mat[r].length-1
                    mat[r][c] = file.nextInt();
                }
            }

            // Find the biggest number in each row, then set the last column to that number for every single row
            for (int r = 0; r < mat.length; r++) {
                int rowBig = Integer.MIN_VALUE; // or can do mat[r][0]
